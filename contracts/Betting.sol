...
contract Betting {
    ...
    constructor(address addr) {
        ConsumerContract = addr;
    }

    function createBettingPool(string memory _name, bytes32 _requestID, uint256 _req_index, uint256 _gameID) public returns (uint256) {
        BettingPool storage pool = bettingPools[poolCount];
        //bettingPools[poolCount] = BettingPool(poolCount, _name, 0, 0, 3, false, _requestID, _req_index, gameID);
        pool.id = poolCount;
        pool.name = _name;
        pool.totalBets = 0;
        pool.totalAmount = 0;
